...
pub fn build(b: *std.Build) void {
    ...
    const sc64_upload_step = b.step("sc64-upload", "Upload the ROM to a USB-connected Summercart 64");
    ...
    const sc64_debug_step = b.step("sc64-debug", "Open debug console on a USB-connected Summercart 64");
    sc64_debug_step.dependOn(&sc64_debug.step);

    const run_ares = b.addSystemCommand(&.{"ares"});
    run_ares.addFileArg(rom);
    const run_ares_step = b.step("run-ares", "Run compiled ROM in an Ares emulator session");
